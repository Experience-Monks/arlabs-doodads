...
namespace Google.XR.ARCoreExtensions.Editor.Internal
{
    ...
    public class CloudAnchorPreprocessBuild : IPreprocessBuildWithReport
    {
        ...
        private const string _locationDependencyGuid = "093f01f1e49684b728a16e550eb72503";

        private const string _locationIosDependencyFileName =
            "ARCoreiOSEarthCloudAnchorDependencies";
        ...
        {
            if (!ARCoreExtensionsProjectSettings.Instance.EarthCloudAnchorEnabled)
            {
                return;
            }
            ...
            {
                CleanupEarthDependencyOnAndroid();
            }
            else if (target == UnityEditor.BuildTarget.iOS)
            {
                AddEarthDependencyOnIos(false);
            }
        }
        ...
        {
            SetApiKeyOnAndroid();
            AddEarthDependecyOnAndroid(
                ARCoreExtensionsProjectSettings.Instance.EarthCloudAnchorEnabled);
        }

        public void PreprocessIOSBuild()
        {
            SetApiKeyOnIOS();
            AddEarthDependencyOnIos(
                ARCoreExtensionsProjectSettings.Instance.EarthCloudAnchorEnabled);
        }
        ...
        private static void CleanupEarthDependencyOnAndroid()
        {
            if (UnityEditorInternal.InternalEditorUtility.inBatchMode &&
                System.Environment.CommandLine.Contains("-earth_cleandeps"))
            {
                Debug.Log("ARCoreExtensions: Cleaning up Earth Cloud Anchor dependencies.");

                // Run the pre-process step with disabled so project files get reset.
                // Then run the ExternalDependencyManager dependency resolution which will remove
                // earth cloud anchor dependencies.
                AddEarthDependecyOnAndroid(false);
